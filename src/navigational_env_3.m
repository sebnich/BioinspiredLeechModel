function navigational_env_3(mFreq,vFreq,color)
...
area_radius = 41.5;
viscircles([area_radius,area_radius],area_radius,'Color','black')
...
start = [13 13 pi/2]; %x,y,r for start


viscircles([g(1) g(2)],gR,'Color','black');
viscircles([start(1) start(2)], aR,'Color',color);
...
viscircles([aPos(1,1) aPos(1,2)],aR,'Color',color);
...
while t < T  
    ...
    for i = 1:N
        ...
        viscircles([aPos(t,1) aPos(t,2)],aR,'Color',color);
        %pause(.1)
        t=t+1;
        flag = 0;
...
for i = 1:length(behavior(:,5))-1
    if behavior(i+1,5) ~= 0
        ...
        ave_ratio = ave_ratio+behavior(i+1,5);
    end    
end
length(b)
ave_ratio/length(b)
